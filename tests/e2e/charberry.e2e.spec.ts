...
test.describe('CharBerry Electron QA', () => {
  ...
  test('supports German UI, SRD creator, structured inventory/session notes, and context menus', async ({}, testInfo) => {
    ...
    try {
      ...
      await expect(page.locator('.play-dashboard')).toHaveAttribute('aria-label', 'Spielübersicht')
      await expect(page.getByRole('button', { name: 'Wahrnehmung' })).toBeVisible()
      await expect(page.getByRole('button', { name: 'GES-Rettung' })).toBeVisible()
      await expect(page.locator('.play-dashboard')).not.toContainText('Play Dashboard')
      await expect(page.locator('.play-dashboard')).not.toContainText('Perception')
      await expect(page.locator('.play-dashboard')).not.toContainText('DEX Save')
